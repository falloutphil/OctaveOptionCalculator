...
class McClass a where
  ...
  payOff  :: a -> MonteCarloUserData -> [Double]
...
singleResult initRngState initNormState initMcState userData = 
   let (((_,mc_s), norm_s), rng_s) = runState rngMonad initRngState
      ...
      in (payOff mc_s userData, (rng_s,norm_s))
...
simState userData initUnderl = 
   State $ \(curSum,(rng_s,norm_s)) -> let (profit,states) = singleResult rng_s norm_s initUnderl userData
...
payOffStandard :: PutCall -> Double -> Double -> Double
